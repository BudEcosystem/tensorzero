...
impl Migration for Migration0031<'_> {
    ...
    fn rollback_instructions(&self) -> String {
        "CREATE TABLE BatchRequest_old (batch_id UUID, id UUID, batch_params String, model_name LowCardinality(String), model_provider_name LowCardinality(String), status Enum('pending' = 1, 'completed' = 2, 'failed' = 3) DEFAULT 'pending', errors Array(String), timestamp DateTime MATERIALIZED UUIDv7ToDateTime(id), raw_request String, raw_response String, function_name LowCardinality(String), variant_name LowCardinality(String), openai_batch_id Nullable(String), completion_window String DEFAULT '24h', input_file_id Nullable(String), output_file_id Nullable(String), error_file_id Nullable(String), created_at Nullable(DateTime), in_progress_at Nullable(DateTime), expires_at Nullable(DateTime), finalizing_at Nullable(DateTime), completed_at Nullable(DateTime), failed_at Nullable(DateTime), expired_at Nullable(DateTime), cancelling_at Nullable(DateTime), cancelled_at Nullable(DateTime), request_counts Nullable(String), metadata Nullable(String)) ENGINE = MergeTree() ORDER BY (batch_id, id); INSERT INTO BatchRequest_old SELECT batch_id, id, batch_params, model_name, model_provider_name, CASE WHEN status IN ('validating', 'in_progress', 'finalizing') THEN 'pending' WHEN status = 'completed' THEN 'completed' WHEN status IN ('failed', 'expired', 'cancelled') THEN 'failed' ELSE CAST(status AS Enum('pending' = 1, 'completed' = 2, 'failed' = 3)) END as status, errors, raw_request, raw_response, function_name, variant_name, openai_batch_id, completion_window, input_file_id, output_file_id, error_file_id, created_at, in_progress_at, expires_at, finalizing_at, completed_at, failed_at, expired_at, cancelling_at, cancelled_at, request_counts, metadata FROM BatchRequest; DROP TABLE BatchRequest; RENAME TABLE BatchRequest_old TO BatchRequest;"
            .to_string()
    }
}
