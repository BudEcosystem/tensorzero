...
async fn test_realtime_session_creation() {
    ...
    assert_eq!(response_json.get("object").unwrap().as_str().unwrap(), "realtime.session");
    assert!(response_json.get("id").unwrap().as_str().unwrap().starts_with("sess_"));
    assert_eq!(response_json.get("model").unwrap().as_str().unwrap(), "gpt-4o-realtime-preview");
    assert_eq!(response_json.get("expires_at").unwrap().as_i64().unwrap(), 0);
    
    // Check client_secret structure
    let client_secret = response_json.get("client_secret").unwrap();
    assert!(client_secret.get("value").unwrap().as_str().unwrap().starts_with("eph_"));
    assert!(client_secret.get("expires_at").unwrap().as_i64().unwrap() > 0);

    // Check default values
    assert_eq!(response_json.get("voice").unwrap().as_str().unwrap(), "alloy");
    assert_eq!(response_json.get("input_audio_format").unwrap().as_str().unwrap(), "pcm16");
    assert_eq!(response_json.get("output_audio_format").unwrap().as_str().unwrap(), "pcm16");
    assert_eq!(response_json.get("temperature").unwrap().as_f64().unwrap(), 0.8);
    assert_eq!(response_json.get("max_response_output_tokens").unwrap().as_str().unwrap(), "inf");
    assert_eq!(response_json.get("tool_choice").unwrap().as_str().unwrap(), "auto");
    ...
    assert_eq!(turn_detection.get("type").unwrap().as_str().unwrap(), "server_vad");
    assert_eq!(turn_detection.get("threshold").unwrap().as_f64().unwrap(), 0.5);
    assert_eq!(turn_detection.get("prefix_padding_ms").unwrap().as_u64().unwrap(), 300);
    assert_eq!(turn_detection.get("silence_duration_ms").unwrap().as_u64().unwrap(), 200);
    assert_eq!(turn_detection.get("create_response").unwrap().as_bool().unwrap(), true);
    assert_eq!(turn_detection.get("interrupt_response").unwrap().as_bool().unwrap(), true);
...
async fn test_realtime_session_creation_with_custom_params() {
    ...
    assert_eq!(response_json.get("voice").unwrap().as_str().unwrap(), "nova");
    assert_eq!(response_json.get("temperature").unwrap().as_f64().unwrap(), 0.5);
    assert_eq!(response_json.get("instructions").unwrap().as_str().unwrap(), "You are a helpful coding assistant.");
    assert_eq!(response_json.get("input_audio_format").unwrap().as_str().unwrap(), "g711_ulaw");
    assert_eq!(response_json.get("output_audio_format").unwrap().as_str().unwrap(), "g711_alaw");
    ...
    let turn_detection = response_json.get("turn_detection").unwrap();
    assert_eq!(turn_detection.get("threshold").unwrap().as_f64().unwrap(), 0.7);
    assert_eq!(turn_detection.get("prefix_padding_ms").unwrap().as_u64().unwrap(), 500);
    assert_eq!(turn_detection.get("silence_duration_ms").unwrap().as_u64().unwrap(), 300);
}
...
async fn test_transcription_session_creation() {
    ...
    // Verify response structure matches OpenAI format
    assert_eq!(response_json.get("object").unwrap().as_str().unwrap(), "realtime.transcription_session");
    assert!(response_json.get("id").unwrap().as_str().unwrap().starts_with("sess_"));
    assert_eq!(response_json.get("model").unwrap().as_str().unwrap(), "gpt-4o-mini-transcribe");
    assert_eq!(response_json.get("expires_at").unwrap().as_i64().unwrap(), 0);
    
    // Check client_secret structure
    let client_secret = response_json.get("client_secret").unwrap();
    assert!(client_secret.get("value").unwrap().as_str().unwrap().starts_with("eph_transcribe_"));
    assert!(client_secret.get("expires_at").unwrap().as_i64().unwrap() > 0);
    ...
    assert_eq!(input_audio_transcription.get("model").unwrap().as_str().unwrap(), "whisper-1");
    assert_eq!(input_audio_transcription.get("language").unwrap().as_str().unwrap(), "en");
    assert_eq!(input_audio_transcription.get("prompt").unwrap().as_str().unwrap(), "Transcribe this conversation");
...
async fn test_realtime_session_invalid_model() {
    ...
    let error_message = error_json.get("error").unwrap().get("message").unwrap().as_str().unwrap();
...
async fn test_max_response_output_tokens_formats() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let response_json = response.json::<Value>().await.unwrap();
    assert_eq!(response_json.get("max_response_output_tokens").unwrap().as_u64().unwrap(), 1000);
...
async fn test_realtime_session_with_tools() {
    ...
    assert_eq!(tool.get("function").unwrap().get("name").unwrap().as_str().unwrap(), "get_weather");
    
    // Check tool_choice
    assert_eq!(response_json.get("tool_choice").unwrap().as_str().unwrap(), "required");
}
...
async fn test_sdk_style_realtime_session() {
    ...
    assert!(session.get("id").unwrap().as_str().unwrap().starts_with("sess_"));
    assert_eq!(session.get("object").unwrap().as_str().unwrap(), "realtime.session");
    ...
    assert_eq!(session.get("model").unwrap().as_str().unwrap(), "gpt-4o-realtime-preview-2024-12-17");
    assert_eq!(session.get("voice").unwrap().as_str().unwrap(), "alloy");
    assert_eq!(session.get("instructions").unwrap().as_str().unwrap(), "You are a friendly assistant.");
    ...
    assert_eq!(td.get("interrupt_response").unwrap().as_bool().unwrap(), true);
...
async fn test_sdk_style_transcription_session() {
    ...
    // Verify the response matches the expected SDK format
    assert!(session.get("id").unwrap().as_str().unwrap().starts_with("sess_"));
    assert_eq!(session.get("object").unwrap().as_str().unwrap(), "realtime.transcription_session");
    assert_eq!(session.get("expires_at").unwrap().as_i64().unwrap(), 0);
